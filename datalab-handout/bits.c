...
// use (!!x) as a representation of (x!=0)
  ...
  // int bitsnum = 0; //use binary search
  // bitsnum += ((!!(x >> (bitsnum + 16))) << 4);
  // bitsnum += ((!!(x >> (bitsnum + 8))) << 3);
  // bitsnum += ((!!(x >> (bitsnum + 4))) << 2);
  // bitsnum += ((!!(x >> (bitsnum + 2))) << 1);
  // bitsnum += (!!(x >> (bitsnum + 1)));
  // // bitsnum += (!!bitsnum) + (~0) + (!(1 ^ x)); //correct when x=0, return -1
  // return bitsnum;
}
/* 
